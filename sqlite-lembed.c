...
int tokenize(
  ...
  int input_token_count_estimate = llama_tokenize(model, input, input_length, NULL, 0, true, true);
  ...
  *tokens = sqlite3_malloc(sizeof(llama_token) * abs(input_token_count_estimate));
  ...
  int input_token_count = llama_tokenize(model, input, input_length, *tokens, abs(input_token_count_estimate), true, true);
...
int embed_single(
  ...
  normalize(llama_get_embeddings_seq(context, batch.seq_id[0][0]), embedding, dimensions);
...
static void lembed_model_size(sqlite3_context *context, int argc, sqlite3_value **argv) {
  struct llama_model * model = sqlite3_value_pointer(argv[0], POINTER_NAME_MODEL);
  if(!model) return;
  sqlite3_result_int64(context, llama_model_size(model));
}

static void lembed_model_options_(sqlite3_context *context, int argc, sqlite3_value **argv) {
  assert(argc >= 0);
  assert(argc % 2 == 0);
...
static void lembed_context_options_(sqlite3_context *context, int argc, sqlite3_value **argv) {
  assert(argc >= 0);
  assert(argc % 2 == 0);
  ...
  sqlite3_result_pointer(context, o, POINTER_NAME_CONTEXT_OPTIONS, sqlite3_free);
}
static char * POINTER_NAME_MODEL_PATH = "lembed_model_path";
static void lembed_model_from_file(sqlite3_context *context, int argc, sqlite3_value **argv) {
  sqlite3_result_pointer(
    context,
    sqlite3_mprintf("%.*s", sqlite3_value_bytes(argv[0]), sqlite3_value_text(argv[0])),
    POINTER_NAME_MODEL_PATH,
    sqlite3_free
  );
}

static void lembed_debug(sqlite3_context *context, int argc, sqlite3_value **argv) {
  sqlite3_result_text(context, sqlite3_mprintf("metal=%d, %s",ggml_cpu_has_metal(), llama_print_system_info()), -1, sqlite3_free);
}

int api_model_from_name(struct Api * api, const char * name, int name_length, struct llama_model ** model, struct llama_context ** context) {
  for(int i = 0; i < MAX_MODELS; i++) {
    if(!api->models[i].name) continue;
...
static void lembed(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  int rc = api_model_from_name(
    (struct Api *) sqlite3_user_data(context),
...
static void lembed_tokenize_json(sqlite3_context *context, int argc, sqlite3_value **argv) {
  struct llama_model * model;
  int rc = api_model_from_name(
    (struct Api *) sqlite3_user_data(context),
    (const char *) sqlite3_value_text(argv[0]),
...
static void lembed_token_score(sqlite3_context *context, int argc, sqlite3_value **argv) {
  struct llama_model * model;
  int rc = api_model_from_name(
    (struct Api *) sqlite3_user_data(context),
    (const char *) sqlite3_value_text(argv[0]),
...
static void lembed_token_to_piece_(sqlite3_context *context, int argc, sqlite3_value **argv) {
  struct llama_model * model;
  int rc = api_model_from_name(
    (struct Api *) sqlite3_user_data(context),
    (const char *) sqlite3_value_text(argv[0]),
...
static void lembed_token_type(sqlite3_context *context, int argc, sqlite3_value **argv) {
  struct llama_model * model;
  int rc = api_model_from_name(
    (struct Api *) sqlite3_user_data(context),
    (const char *) sqlite3_value_text(argv[0]),
...
static void ggml_test(sqlite3_context *context, int argc, sqlite3_value **argv) {
...
#define LEMBED_MODELS_CONTEXT_OPTIONS 3
  rc = sqlite3_declare_vtab(
      db, "CREATE TABLE x(key, model, model_options hidden, context_options hidden)");
  if (rc == SQLITE_OK) {
...
static int lembed_modelsUpdate(sqlite3_vtab *pVTab, int argc, sqlite3_value **argv,
                      sqlite_int64 *pRowid) {
  lembed_models_vtab* p  = (lembed_models_vtab*) pVTab;
  // DELETE operation
  ...
  // INSERT operation
    ...
    const char * key = (const char *) sqlite3_value_text(columnValues[LEMBED_MODELS_KEY]);
    int idx = -1;
    for(int i = 0; i < MAX_MODELS; i++) {
    ...
    const char * modelPath = sqlite3_value_pointer(columnValues[LEMBED_MODELS_MODEL], POINTER_NAME_MODEL_PATH);
    assert(modelPath);

    lembed_model_options * modelOptions = NULL;
    if(sqlite3_value_subtype(columnValues[LEMBED_MODELS_MODEL_OPTIONS]) == POINTER_SUBTYPE) {
      modelOptions = sqlite3_value_pointer(columnValues[LEMBED_MODELS_MODEL_OPTIONS], POINTER_NAME_MODEL_OPTIONS);
    }

    lembed_context_options * contextOptions = NULL;
    if(sqlite3_value_subtype(columnValues[LEMBED_MODELS_CONTEXT_OPTIONS]) == POINTER_SUBTYPE) {
      contextOptions = sqlite3_value_pointer(columnValues[LEMBED_MODELS_CONTEXT_OPTIONS], POINTER_NAME_CONTEXT_OPTIONS);
    }
...
static int lembed_modelsBestIndex(sqlite3_vtab *pVTab, sqlite3_index_info *pIdxInfo) {
  pIdxInfo->idxNum = 1;
...
static int lembed_modelsColumn(sqlite3_vtab_cursor *cur, sqlite3_context *context,
                         int i) {
  lembed_models_cursor *pCur = (lembed_models_cursor *)cur;
  lembed_models_vtab *p = (lembed_models_vtab *)cur->pVtab;
  switch (i) {
  case LEMBED_MODELS_KEY:
    sqlite3_result_text(context, p->api->models[pCur->iRowid].name, -1, SQLITE_TRANSIENT);
    break;
  case LEMBED_MODELS_MODEL:
    sqlite3_result_pointer(context, p->api->models[pCur->iRowid].model, POINTER_NAME_MODEL, NULL);
    break;
  }
  return SQLITE_OK;
}
...
#define lembed_chunks_CHUNK_SIZE   3
  rc = sqlite3_declare_vtab(
      db, "CREATE TABLE x(contents, token_count, source hidden, chunk_size hidden)");
  if (rc == SQLITE_OK) {
    pNew = sqlite3_malloc(sizeof(*pNew));
    *ppVtab = (sqlite3_vtab *)pNew;
    if (pNew == 0)
...
static int lembed_chunksBestIndex(sqlite3_vtab *pVTab, sqlite3_index_info *pIdxInfo) {
  int hasSource = 0;
  int idxChunkSize = -1;
...
static int lembed_chunksFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
  ...
  for(int i = 0; i < nchunks; i++) {
    ...
    for(int j = 0; j < chunk_size; j++) {
      ...
      int32_t piece_len_neg = llama_token_to_piece(model, token, NULL, 0, false);
      //printf("%d\n", piece_len_neg);
...
static int lembed_chunksColumn(sqlite3_vtab_cursor *cur, sqlite3_context *context,
                         int i) {
  lembed_chunks_cursor *pCur = (lembed_chunks_cursor *)cur;
  switch (i) {
...
int sqlite3_lembed_init(sqlite3 *db, char **pzErrMsg,
  ...
  sqlite3_create_function_v2(db, "lembed_debug",  0, SQLITE_UTF8,                         a, lembed_debug, NULL, NULL, NULL);

  sqlite3_create_function_v2(db, "lembed",                     2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed,                 NULL, NULL, NULL);
  sqlite3_create_function_v2(db, "lembed_tokenize_json",       2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_tokenize_json,   NULL, NULL, NULL);
  sqlite3_create_function_v2(db, "lembed_token_score",         2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_token_score,     NULL, NULL, NULL);
  sqlite3_create_function_v2(db, "lembed_token_type",          2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_token_type,      NULL, NULL, NULL);
  sqlite3_create_function_v2(db, "lembed_token_to_piece",      2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_token_to_piece_, NULL, NULL, NULL);

  sqlite3_create_function_v2(db, "lembed_model_size",      1, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_model_size, NULL, NULL, NULL);

  sqlite3_create_function_v2(db, "lembed_model_from_file",           1, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_model_from_file,      NULL, NULL, NULL);
  sqlite3_create_function_v2(db, "lembed_model_options",            -1, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_model_options_,      NULL, NULL, NULL);
  sqlite3_create_function_v2(db, "lembed_context_options",          -1, SQLITE_UTF8 | SQLITE_DETERMINISTIC,  a, lembed_context_options_,      NULL, NULL, NULL);

  sqlite3_create_function_v2(db, "ggml_test", 0, SQLITE_UTF8 | SQLITE_DETERMINISTIC, a, ggml_test, NULL, NULL, NULL);
